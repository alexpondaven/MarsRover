  ...
  if (int_count == 1000) { // SLOW LOOP (1Hz)
    ...
    switch (state_num) { // STATE MACHINE (see diagram)
      ...
      case 1:{ // Charge state (250mA and a green LED)
        ...
        if (rest_timer < 10) { // if not charged, stay put
          //digitalWrite(4,true);
          next_state = 1;
          digitalWrite(8,true); 
          rest_timer++;         
      ...
      case 2:{ // Charge Rest, green LED is off and no current
        current_ref = 0;
        if (rest_timer_rest < 30) { // Stay here if timer < 30
          digitalWrite(5, true);
          digitalWrite(4, true);
          next_state = 2;
          rest_timer_rest ++;
      ...
      case 3:{ //Discharge state (-250mA and both LEDs)
         current_ref = 0;
    ...
    dataString = String(state_num) + "," + String(V_measure_1) + "," + String (V_measure_2)+ "," + String(current_ref) + "," + String(current_measure); //build a datastring for the CSV file
