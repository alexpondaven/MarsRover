...
unsigned int DdSpeed, DdPosition, dVolt; //the speed and position need to be achieved
unsigned int sensorValue0, sensorValue1, sensorValue2, sensorValue3; // ADC sample values declaration
float ev = 0, cv = 0, ei = 0, oc = 0, ey = 0, cy = 0; //internal signals
float Ts = 0.0008; //1.25 kHz control frequency. It's better to design the control period as integral multiple of switching period.
float kpv = 0.05024, kiv = 15.78, kdv = 0; // voltage pid.
float u0v, u1v, delta_uv, e0v, e1v, e2v; // u->output; e->error; 0->this time; 1->last time; 2->last last time
float kpi = 0.02512, kii = 39.4, kdi = 0; // current pid.
float u0y, u1y, delta_uy, e0y, e1y, e2y;
float kpy = 0.02512, kiy = 39.4, kdy = 0;
...
//  }
  ...
  if (!digitalRead(Reset) > 0) {
    ...
    Serial.println("reset_the_mousecam");
  ...
  */
  ...
  Serial.println("total_x1 = " + String(total_x1));

  Serial.println("Distance_y = " + String(total_y));
  Serial.println("total_y1 = " + String(total_y1));
  Serial.print('\n');
  ...
  if (messageReady) {
    ...
    delay(10);
    if (error) {
      Serial.print(F("deserialieJson() Failed: "));
      messageReady = false;
      return;
    }
    if (doc["Type"] == "request" ) {
  ...
  if (buf == forword1) {
    ...
    //  DIRLstate = LOW;
    //  findsubchar(1,buf);
    //  Serial.print("seeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee:");
    ...
    int control_x1 = 0;
    int control_x2 = 0;
    control_x1 = (control_x1 + distance_x);
    while (control_x1 < 1000) {
      dVolt = 500;
      DIRRstate = HIGH;
      DIRLstate = HIGH;
      digitalWrite(DIRR, DIRRstate);
      digitalWrite(DIRL, DIRLstate);
      Serial.print(control_x1);

    }
    dVolt = 0;

  }
  else if (buf == backword) {
    dVolt = 500;
    DIRRstate = LOW;
    DIRLstate = HIGH;
...
void sampling() {
  ...
  sensorValue2 = dVolt; //sample Vref
...
// This is a PID controller for the distance
int xControl(int x) {
  if (x > 2) {
    DIRRstate = HIGH;
    DIRLstate = HIGH;
