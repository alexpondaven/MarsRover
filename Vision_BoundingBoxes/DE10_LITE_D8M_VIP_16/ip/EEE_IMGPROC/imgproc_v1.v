...
// Detect red areas
wire red_detect, pink_detect, green_detect, greenblue_detect, blue_detect, yellow_detect;
assign red_detect = red[7] & ~green[7] & ~blue[7]; // alright, but pink is also majorly covered
//if there are 2 red BBs, could just take bigger one?
assign pink_detect = red[7] & ~green[7] & green[6] & ~blue[7] & blue[6]; // good (sometimes only when viewed at an angle), but red ball still detected slightly



assign green_detect = ~red[7] & green[7] & ~blue[7]; // only detects edge
assign greenblue_detect = ~red[7] & green[7] & ~blue[7] & blue[6]; // pretty similar to green, also colour is hard to see

assign blue_detect = ~red[7] & ~green[7] & blue[7]; //only works if gain is increased a lot
assign yellow_detect = red[7] & green[7] & ~blue[7]; // too good, almost entire table is covered

// Find boundary of cursor box

// Highlight detected areas
wire [23:0] red_high, pink_high, green_high, greenblue_high, blue_high, yellow_high;
...
assign greenblue_high = greenblue_detect ? {8'h0, 8'h91, 8'h61} : {grey, grey, grey};

assign blue_high = blue_detect ? {8'h0, 8'h00, 8'hff} : {grey, grey, grey};
...
assign {red_out, green_out, blue_out} = ((mode==3'b001) & ~sop & packet_video) ? red_high:
														((mode==3'b011) & ~sop & packet_video) ? pink_high:
														((mode==3'b010) & ~sop & packet_video) ? green_high:
														((mode==3'b110) & ~sop & packet_video) ? greenblue_high:
														((mode==3'b111) & ~sop & packet_video) ? blue_high:
														((mode==3'b101) & ~sop & packet_video) ? yellow_high:
														((mode==3'b100) & ~sop & packet_video) ? red_high:
																										     {red,green,blue};
