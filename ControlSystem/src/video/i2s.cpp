...
#define BUFFER_LINES_READ 60


static char video_frame_buff[FRAME_SIZE_BYTES];
...
void init_i2s(void)
{
    ...
    PIN_FUNC_SELECT(PIN_CTRL, CLK_OUT2_S);
    ...
    xTaskCreate(get_frame_i2s, "Read frame I2S", 1024, NULL, READ_VIDEO_FRAME_PRIORITY, NULL);
}
...
void get_frame_i2s(void* params) {
  ...
  while (1) {
    ...
    for (int i=0; i<(FRAME_SIZE_BYTES/(LINE_SIZE_BYTES * BUFFER_LINES_READ)) ; i++ ) {
      // wait until half the buffer has data and read
      i2s_read(I2S_NUM_0, &video_frame_buff[total_bytes_read], LINE_SIZE_BYTES * BUFFER_LINES_READ, &bytes_read, portMAX_DELAY);
      gpio_set_level(I2S_CLEAR_TO_SEND, 0);
    ...
    ESP_LOGI(TAG, "First 2 pixels: %h", *((uint8_t *)video_frame_buff[0]), *((uint8_t *)video_frame_buff[1]), *((uint8_t *)video_frame_buff[2]));
    ESP_LOGI(TAG, "Last 2 pixels: %h", *((uint8_t *)video_frame_buff[total_bytes_read-3]), *((uint8_t *)video_frame_buff[total_bytes_read-2]), *((uint8_t *)video_frame_buff[total_bytes_read-1]));
