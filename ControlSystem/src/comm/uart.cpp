...
void uart_power_arduino(void *params) {
  ...
  ESP_ERROR_CHECK(uart_driver_install(UART_NUM_2, sizeof(power_rx_data), 0, 1, &uart_queue_power, 0));
...
void uart_drive_arduino(void *params) {
  ...
  ESP_ERROR_CHECK(uart_driver_install(UART_NUM_1, sizeof(drive_rx_data), sizeof(drive_tx_data), 10, &uart_queue_drive, 0));
...
void uart_setup() {
  ...
  xTaskCreate(uart_drive_arduino, "Drive Arduino UART", 1024, NULL, DRIVE_ARDUINO_COMM_PRIORITY, NULL);
  xTaskCreate(uart_power_arduino, "Power Arduino UART", 1024, NULL, POWER_ARDUINO_COMM_PRIORITY, NULL);
}
